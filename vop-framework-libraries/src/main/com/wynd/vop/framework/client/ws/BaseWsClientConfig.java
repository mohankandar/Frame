...
import com.wynd.vop.framework.exception.BipPartnerRuntimeException;
...
import com.wynd.vop.framework.security.user.ActivePersonTraitsUserInformationProvider;
...
public class BaseWsClientConfig {
	...
	public final Jaxb2Marshaller getMarshaller(final String transferPackage, final Resource[] schemaLocations,
		...
		try {
			...
			throw new BipPartnerRuntimeException(MessageKeys.VOP_REST_CONFIG_JAXB_MARSHALLER_FAIL,
	...
	protected final VAServiceWss4jSecurityInterceptor getVAServiceWss4jSecurityInterceptor(final String username,
		...
		LOGGER.debug("Fetching VAServiceWss4J for Username: {} in App: {}", username, vaApplicationName);
	...
	protected final VAServiceWss4jSecurityInterceptor getVAServiceWss4jSecurityInterceptor(String username,
		...
		VAServiceWss4jSecurityInterceptor interceptor = getVAServiceWss4jSecurityInterceptor(
				new StaticUserInformationProvider(vaApplicationName, username, station),
				password,
				includeExternalVAHeaders
		);
		LOGGER.debug("Fetched VAServiceWss4J for Username: {} in App: {}", username, vaApplicationName);
		return interceptor;
	}

	/**
	 * Gets the security interceptor for an authenticated {@link PersonTraits} user. The
	 * interceptor will authenticate each request with the currently authenticated user rather than a static value.
	 *
	 * @param pass-word
	 * @param includeExternalVAHeaders whether to include the VA External Headers in the soap header
	 * @return the security interceptor
	 */
	protected final VAServiceWss4jSecurityInterceptor getActiveUserVAServiceWss4jSecurityInterceptor(
			final String password,
			final boolean includeExternalVAHeaders) {
		LOGGER.debug("Fetching VAServiceWss4J for authenticated user.");
		return getVAServiceWss4jSecurityInterceptor(new ActivePersonTraitsUserInformationProvider(),
				password,
				includeExternalVAHeaders);
	}
	 ...
	 * Gets the security interceptor.
	 *
	 * @param userInformationProvider the {@link UserInformationProvider} for the interceptor
	 * @param pass-word
	 * @param includeExternalVAHeaders whether to include the VA External Headers in the soap header
	 * @return the security interceptor
	 */
	protected final VAServiceWss4jSecurityInterceptor getVAServiceWss4jSecurityInterceptor(
			final UserInformationProvider userInformationProvider,
			final String password,
			final boolean includeExternalVAHeaders) {

		final VAServiceWss4jSecurityInterceptor interceptor = new VAServiceWss4jSecurityInterceptor();
		interceptor.setUserInformationProvider(userInformationProvider);
		...
		interceptor.setUseRequestAttributesForClientMachine(useRequestClientMachine);
		return interceptor;
	}
}
